...
@CrossOrigin
@RestController
@RequestMapping(name = "Tjeneste", value = RestEndpoints.TJENESTE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class TjenesteController {

    @Autowired(required = false)
    private TjenesteCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Tjeneste cache is disabled.");
        }
    ...
    @GetMapping
    public TjenesteResources getTjeneste(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.GET_ALL_TJENESTE, client);
        ...
        Stream<TjenesteResource> resources;
    ...
    public TjenesteResource getTjenesteBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.GET_TJENESTE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<TjenesteResource> tjeneste = cacheService.getTjenesteBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return tjeneste.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            TjenesteResource tjeneste = objectMapper.convertValue(response.getData().get(0), TjenesteResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.toResource(tjeneste);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        switch (event.getResponseStatus()) {
            case ACCEPTED:
                ...
                result = objectMapper.convertValue(event.getData().get(0), TjenesteResource.class);
    ...
    @PostMapping
    public ResponseEntity postTjeneste(
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody TjenesteResource body,
            @RequestParam(name = "validate", required = false) boolean validate
    ) {
        log.debug("postTjeneste, Validate: {}, OrgId: {}, Client: {}", validate, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.UPDATE_TJENESTE, client);
    ...
    public ResponseEntity putTjenesteBySystemId(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody TjenesteResource body
    ) {
        log.debug("putTjenesteBySystemId {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.UPDATE_TJENESTE, client);
        event.setQuery("systemid/" + id);

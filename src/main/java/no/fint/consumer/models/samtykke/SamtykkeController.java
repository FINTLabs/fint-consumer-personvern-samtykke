...
@CrossOrigin
@RestController
@RequestMapping(name = "Samtykke", value = RestEndpoints.SAMTYKKE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class SamtykkeController {

    @Autowired(required = false)
    private SamtykkeCacheService cacheService;
    ...
    @GetMapping
    public SamtykkeResources getSamtykke(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.GET_ALL_SAMTYKKE, client);
        ...
        Stream<SamtykkeResource> resources;
    ...
    public SamtykkeResource getSamtykkeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.GET_SAMTYKKE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<SamtykkeResource> samtykke = cacheService.getSamtykkeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return samtykke.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            SamtykkeResource samtykke = objectMapper.convertValue(response.getData().get(0), SamtykkeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.toResource(samtykke);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        switch (event.getResponseStatus()) {
            case ACCEPTED:
                ...
                result = objectMapper.convertValue(event.getData().get(0), SamtykkeResource.class);
    ...
    @PostMapping
    public ResponseEntity postSamtykke(
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody SamtykkeResource body,
            @RequestParam(name = "validate", required = false) boolean validate
    ) {
        log.debug("postSamtykke, Validate: {}, OrgId: {}, Client: {}", validate, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.UPDATE_SAMTYKKE, client);
    ...
    public ResponseEntity putSamtykkeBySystemId(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody SamtykkeResource body
    ) {
        log.debug("putSamtykkeBySystemId {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.UPDATE_SAMTYKKE, client);
        event.setQuery("systemid/" + id);

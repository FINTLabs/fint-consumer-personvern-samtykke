...
@CrossOrigin
@RestController
@RequestMapping(name = "Behandling", value = RestEndpoints.BEHANDLING, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class BehandlingController {
    ...
    @GetMapping
    public BehandlingResources getBehandling(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.GET_ALL_BEHANDLING, client);
        ...
        Stream<BehandlingResource> resources;
    ...
    public BehandlingResource getBehandlingBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, SamtykkeActions.GET_BEHANDLING, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<BehandlingResource> behandling = cacheService.getBehandlingBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return behandling.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            BehandlingResource behandling = objectMapper.convertValue(response.getData().get(0), BehandlingResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.toResource(behandling);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        switch (event.getResponseStatus()) {
            case ACCEPTED:
                ...
                result = objectMapper.convertValue(event.getData().get(0), BehandlingResource.class);
